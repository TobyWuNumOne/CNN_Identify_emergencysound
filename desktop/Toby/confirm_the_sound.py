...
class Audiowave:
    ...
    def waveopen(self, filedir):
        ...
        print("總幀數：" + str(self.nframes) + "幀")
        print("採樣率：" + str(self.framerate) + "幀/s")
        print("聲道數：" + str(self.wavechannel) + "個")
        print("位深：" + str(self.wavewidth * 8) + "bit")
        print("比特率：" + str(bps / 1000) + "kbps")
        print("時間：" + str(self.time) + "s")
    ...
    def wavehex_to_DEC(self, wavedata, wavewidth, wavechannel):
        ...
        x = (
            np.linspace(0, len(self.Timedata[0]) - 1, len(self.Timedata[0]))
            / self.framerate
        )
    ...
    def micdata(self):  # 錄音數據，存儲，播放
        ...
        self.data = data
        filter_data = self.filter(data, 0, 600)  ###這個要轉成16進制進行播放
        # print(filter_data)
        # print(filter_data)
...
# a=Audiowave().wavedraw1(r"E:\pythonProject2\wav\test.wav")
# a=Audiowave().wavedrawall(r"E:\pythonProject2\wav\P月23日下午1點03分.wav")
# a=Audiowave().Dynamic_run(r"E:\pythonProject2\wav\test.wave")
# b=b'\xfc\xff\x0b\x00\x11\x00\x10\x00\r\x00\x0c\x00\x0b\x00\t\x00\x08\x00\x08\x00\x06\x00\x06\x00\x04\x00\x03\x00\x02\x00\x00\x00\x00\x00\xfe\xff\xfd\xff\xfb\xff\xff'
#
# a=Audiowave().wavehex_to_DEC(b,2,2)
# print(a)
